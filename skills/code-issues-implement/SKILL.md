...
- Use `$testing-standards` when deciding whether to add or update regression tests for the fix. Add or update the narrowest credible coverage when an established harness owns the changed behavior.
- For behavior-changing fixes, use the same issue execution fields before editing and at completion: `Behavior`, `Coverage`, `Change`, `Refactor`, and `Validation`. Before editing, record the expected behavior, coverage plan, intended change, refactor checkpoint, and planned validation without prescribing whether tests or implementation must be written first.
