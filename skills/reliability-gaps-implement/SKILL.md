...
- For behavior-changing fixes, use the same reliability execution fields before editing and at completion: `Behavior`, `Coverage`, `Change`, `Refactor`, and `Validation`. Before editing, record the expected behavior, coverage plan, intended change, refactor checkpoint, and planned validation without prescribing whether tests or implementation must be written first.
