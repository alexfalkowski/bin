# Code Issues Find Plan

Use this reference to instantiate the code-issue-specific active plan for
`$code-issues-find`. Read it with the shared gap workflow named in `SKILL.md`;
that shared gap workflow owns common plan state, optional goal state,
scoped-ledger, delegation, and coverage gates.

Track code-issue-specific state for scope, delegation, ledger state, code/
security/compatibility evidence, and public contract evidence.
