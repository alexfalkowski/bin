...
7. Add or update Go tests when behavior changes and the repository supports tests.
8. Before writing any Go `*_test.go` file, choose the test package deliberately. Default to an external `<package>_test` package. If you think the test must use the production package instead, stop and explain why the public or documented entrypoint cannot cover the behavior before writing that test.
9. Pair this skill with `$testing-standards` when designing, reviewing, or refactoring Go test coverage.
10. Pair this skill with `$change-validation` when selecting Go test, lint, coverage, or benchmark commands.
