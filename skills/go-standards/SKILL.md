...
8. Add or recommend Go `*_test.go` coverage only when the nearby majority relevant tests for that behavior are Go-based, the changed surface is a Go package/API contract that cannot be covered through the dominant harness, or the repository explicitly asks for Go-level coverage. Agents MUST stop and explain before creating Go tests when another harness owns the behavior.
