...
- Before running, retrying, replacing, or recommending commands, establish the repository root, documented entrypoint, CI analogue, configured command environment, and any platform-specific tool-path setup.
...
- In this shared `bin` repo and downstream repos that vendor it as `./bin`, `make` targets are the preferred validation interface. If `make` reports an unexpected tool or version problem in the agent environment but works in the configured command environment, report the mismatch and keep the Makefile target as the intended command.
