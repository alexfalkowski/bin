...
- Use `$testing-standards` for test design and pair with the relevant language standard for local idioms.
- For test gaps that require behavior-changing production code, use the same test execution fields before editing and at completion: `Behavior`, `Coverage`, `Change`, `Refactor`, and `Validation`. Before editing, record the expected behavior, coverage plan, intended change, refactor checkpoint, and planned validation without prescribing whether tests or implementation must be written first.
