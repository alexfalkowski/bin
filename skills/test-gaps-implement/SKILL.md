...
description: Use when the human says Approved <ID>-N or Start <ID> for a test-gap ledger entry, approves a same-prefix batch such as Approved <ID>-N[/N...], or asks what the fix is for a test-gap ledger entry. Implement agreed test-gap fixes sequentially after explicit agreement, including contract-driven same-prefix approved batches; never starts work without that agreement.
---
...
workflow processes an approved batch sequentially. Use `$test-gaps-find` first
if no scoped ledger entry exists yet for this scope.
